...
export const POST = withAuth(async (req: NextRequest, user) => {
  try {
    ...
    // Detect Windows-1252 encoding from OFX header (common in Brazilian bank exports)
    ...
    for (const row of rows) {
      try {
        const existing = await prisma.transaction.findFirst({ where: { householdId: user.householdId, notes: `ofx:${row.fitid}` } });
        if (existing) continue;
        const tx = await prisma.transaction.create({
          data: { householdId: user.householdId, type: row.amount >= 0 ? 'INCOME' : 'EXPENSE', amount: Math.abs(row.amount), description: row.name || row.memo || null, date: row.date, isPaid: true, accountId, notes: `ofx:${row.fitid}` },
        });
        created.push(tx);
      } catch {
